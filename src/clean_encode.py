...
def clean_encode():
    encode_df = pl.read_parquet('encode.parquet')
    ...
    encode_df = encode_df.drop(pl.col("quality_metrics", "step_run", "analysis_step_version"))
    clean_encode = encode_df

    clean_encode.write_parquet('clean_encode.parquet')
    
    return os.path.exists('clean_encode.parquet')