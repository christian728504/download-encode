...
def get_file_accessions():
    url = 'https://www.encodeproject.org/search/?type=Experiment&format=json&limit=762008'
    ...
    for experiment in tqdm(experiment_graph, desc='Getting file accessions'):
        ...
        experiment_files = [file.get('@id').split('/')[-1] for file in experiment_files]
...
def _process_url(url):
    ...
    while not row:
        if tries > 10:
            sys.exit(f"Failed to get file {url}")
        tries += 1
        headers = {'accept': 'application/json'}
        response = requests.get(url, headers=headers)
        try:
            ...
            print(f"Failed to get file {url}, retrying")
            continue
        file = Box(file)
        row = {
...
def construct_matrix():
    ...
    for accession in accessions:
        urls.append(f"https://www.encodeproject.org/{accession}/?format=json")
    
    rows = []
    with Pool(processes=30) as pool:
        for row in tqdm(pool.imap(func=_process_url, iterable=urls), total=len(urls), desc='Constructing matrix from files json objects'):
            rows.append(row)
